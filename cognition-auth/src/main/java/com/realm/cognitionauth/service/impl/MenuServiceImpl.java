...
import com.realm.cognitionauth.dao.MenuDao;
import com.realm.cognitionauth.dao.RoleMenuDao;
import com.realm.cognitionauth.entity.Menu;
import com.realm.cognitionauth.service.MenuService;
import com.realm.cognitioncommon.persistence.Tree;
import com.realm.cognitioncommon.until.BuildTree;
import com.realm.cognitioncommon.until.Query;
...
public class MenuServiceImpl implements MenuService {
    ...
    public Tree<Menu> getTree(String id) {
        ...
        for (Menu sysMenuDO : menus) {
            ...
            Tree<Menu> tree = BuildTree.setTrees(sysMenuDO.getId().toString(), sysMenuDO.getParentId().toString(), sysMenuDO.getName(), state);
            trees.add(tree);
        }
        // 默认顶级菜单为０，根据数据库实际情况调整
    ...
    private List<Tree<Menu>> treeFormation(List<Menu> menuList) {
        ...
        for (Menu sysMenuDO : menuList) {
            ...
            Tree<Menu> tree = BuildTree.setTrees(sysMenuDO.getId().toString(), sysMenuDO.getParentId().toString(), sysMenuDO.getName(), null);
